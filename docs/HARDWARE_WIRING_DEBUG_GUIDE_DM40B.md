...
## Overview
This guide helps verify physical connections for Adafruit TCA9548A I2C multiplexer breakout and GY-BNO085 IMU before troubleshooting I2C communication issues.
...
### 1.2 Verify Adafruit TCA9548A Power Connections
**ESP32-S3 OFF**, measure continuity:

| Test Point | Expected Resistance | Multimeter Mode | Notes |
|------------|---------------------|-----------------|-------|
| TCA9548A **VCC** → ESP32-S3 **3V3** | 0Ω (short) | Resistance (200Ω range) | Direct connection |
| TCA9548A **GND** → ESP32-S3 **GND** | 0Ω (short) | Resistance (200Ω range) | Direct connection |
| TCA9548A **VCC** → **GND** | >10kΩ (no short) | Resistance (20kΩ range) | Should NOT be shorted |
...
⚠️ **Note**: Ch5 (GY-BNO085) sub-bus has **5.1kΩ** pull-ups installed (SD5/SC5→3.3V). Ch0-4 (TMAG5273) sub-bus pull-ups (4.7kΩ) NOT installed yet. Main bus 2kΩ also passes through TCA9548A internal switches — Ch0-4 sub-buses are functional for testing without dedicated pull-ups.

---

## Step 3: I2C Bus Connectivity (ESP32-S3 OFF)

### 3.1 Verify GPIO → TCA9548A Connections
Measure continuity with **ESP32-S3 OFF**, multimeter in **Resistance mode (200Ω range)**:

| Test Point | Expected Resistance | Meaning |
|------------|---------------------|---------|
| **GPIO8** (ESP32) → **SDA** (TCA9548A) | 0Ω (short) | SDA bus connected |
| **GPIO9** (ESP32) → **SCL** (TCA9548A) | 0Ω (short) | SCL bus connected |
...
## Step 4: TCA9548A Reset Pin (ESP32-S3 OFF)

### 4.1 Verify /RST Connection
TCA9548A has an active-low reset pin. It must be pulled HIGH for normal operation.

| Test Point | Expected Resistance | Notes |
|------------|---------------------|-------|
| TCA9548A **/RST** → **3.3V** | 0Ω (short) or 10kΩ pull-up | Must be HIGH to operate |
| TCA9548A **/RST** → **GND** | >10kΩ (open) | Should NOT be grounded |
...
| Test Point | Expected Resistance | Meaning |
|------------|---------------------|---------|
| TCA9548A **SD5** → BNO085 **SDA** | 0Ω (short) | Channel 5 downstream bus |
| TCA9548A **SC5** → BNO085 **SCL** | 0Ω (short) | Channel 5 downstream bus |
...
| PS0 | PS1 | I2C Address (datasheet) | Notes |
|-----|-----|-------------------------|-------|
| GND | GND | 0x4A | Datasheet default |
| GND | VCC | 0x4B | Alternate |
| VCC | GND | 0x4C | Alternate |
| VCC | VCC | 0x4D | Alternate |

**Actual finding (2026-05-25)**: The 7Semi GY-BNO085 module responds at **0x4B** even with PS0/PS1 grounded. This may be due to module-specific wiring or internal pull-up configuration. Use **0x4B** in firmware.
...
✅ **Pass criteria**: Address pins grounded → verify actual address via I2C scan (expected: **0x4B** for this module)
...
### 6.2 Check TCA9548A Channel 5 Idle State
**ESP32-S3 ON**, measure downstream bus after mux initialization attempt:

| Test Point | Expected Voltage | Meaning |
|------------|------------------|---------|
| TCA9548A **SD5** vs GND | 3.3V (HIGH) | CH5 bus idle |
| TCA9548A **SC5** vs GND | 3.3V (HIGH) | CH5 bus idle |

⚠️ **Note**: These measurements are only valid if TCA9548A responds to I2C. If mux doesn't respond, downstream channels may not be activated.

---

## Troubleshooting Checklist

### If I2C scanner shows "No device at 0x70 (TCA9548A)"

1. ✅ Check TCA9548A VCC/GND power connections (Step 1.2)
2. ✅ Check GPIO8/9 → TCA9548A SDA/SCL continuity (Step 3.1)
3. ✅ Check 2kΩ pull-up resistors (Step 2.1)
4. ✅ Check TCA9548A /RST pin connection to 3.3V (Step 4.1)
5. ✅ Measure GPIO8/9 idle voltage = 3.3V (Step 6.1)

### If I2C scanner shows TCA9548A OK but BNO085 fails

1. ✅ Check BNO085 VCC/GND power connections (Step 5.1)
2. ✅ Check TCA9548A SD5/SC5 → BNO085 SDA/SCL (Step 5.2)
3. ✅ Check BNO085 PS0/PS1 address pins (Step 5.3)
4. ✅ Verify TCA9548A CH5 is being selected in firmware (`MuxChannels::BNO085_IMU` = 5)
...
|------------|------------------------|------------|
| ESP32-S3 **GPIO8** → TCA9548A **SDA** | — | Step 3.1 |
| ESP32-S3 **GPIO9** → TCA9548A **SCL** | — | Step 3.1 |
| 3.3V → 2kΩ → GPIO8 | — | Step 2.1 |
| 3.3V → 2kΩ → GPIO9 | — | Step 2.1 |
| TCA9548A **SD5** → BNO085 **SDA** | — | Step 5.2 |
| TCA9548A **SC5** → BNO085 **SCL** | — | Step 5.2 |
...
4. If TCA9548A still fails → Simulation mode allows development to continue

---

Generated: 2026-05-20
Project: EdgeAI Data Glove V3
Hardware: ESP32-S3-DevKitC-1 N16R8 + Adafruit TCA9548A + 7Semi GY-BNO085