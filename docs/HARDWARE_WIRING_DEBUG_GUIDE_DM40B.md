...
## Overview
This guide helps verify physical connections for TCA9548A I2C multiplexer and BNO085 IMU before troubleshooting I2C communication issues.
...
- 5.1kΩ resistors (×2 for main I2C bus pull-ups)
...
### 1.2 Verify TCA9548A Power Connections
...
Your configuration uses **5.1kΩ pull-ups on main bus only**:

```
3.3V ──[5.1kΩ]── GPIO8 (SDA)
3.3V ──[5.1kΩ]── GPIO9 (SCL)
```
...
|------------|---------------------|---------|
| **GPIO8** → **3.3V** | ~5.1kΩ | SDA pull-up present |
| **GPIO9** → **3.3V** | ~5.1kΩ | SCL pull-up present |
| **GPIO8** → **GND** | >10kΩ (open) | No short to ground |
| **GPIO9** → **GND** | >10kΩ (open) | No short to ground |

✅ **Pass criteria**: Both pull-ups measure ~5.1kΩ, no shorts to ground

⚠️ **Note**: Sub-channel pull-ups (4.7kΩ on TMAG5273 CH0-4) are NOT needed yet since those sensors are not connected. The TCA9548A downstream channels (SD0-SD4) will inherit pull-ups from upstream devices when sensors are added.
...
3. ✅ Check 5.1kΩ pull-up resistors (Step 2.1)
...
| 3.3V → 5.1kΩ → GPIO8 | — | Step 2.1 |
| 3.3V → 5.1kΩ → GPIO9 | — | Step 2.1 |
...
Hardware: ESP32-S3-DevKitC-1 N16R8 + TCA9548A + BNO085