...
- **Sensors**: 5× TMAG5273 (via Adafruit TCA9548A mux), 1× BNO085 (address 0x4B)
...
| GitHub MCP | Working (after restart) | PRs, issues, code search — needs `GITHUB_PERSONAL_ACCESS_TOKEN` in `.Codex/settings.local.json` |
