...
| 2026-05-25 | Adafruit TCA9548A 模块也 **损坏** — 即使用万用表验证了正确接线（VIN=3.14V, RST=2.98V, SDA=3.06V）仍全部 NACK |
| 2026-05-25 | **BNO085 确认可用**，地址为 **0x4B**（非 0x4A），通过直连旁路（移除 TCA9548A）验证。SHTP 通信正常。 |

**当前硬件状态：**
- **BNO085**: 在地址 **0x4B** 正常工作（直连）。固件已更新。
- **Adafruit TCA9548A**: **已损坏** — 第 2 块坏的多路复用器模块。需要重新购买。
- **TMAG5273 霍尔传感器**: 尚未安装（等待 TCA9548A 更换）
- **I2C 总线**: 确认可正常工作（ESP32 GPIO8/9，2kΩ 上拉，面包板）

**需要操作**: 购买替换的 Adafruit TCA9548A 1-to-8 I2C 多路复用器扩展板
