...
| 2 | S3 I2C 扫描（3 设备） | ✅ 完成 | 0x48+0x49+0x4B 全部检测，扁平总线确认 |
| 3 | BNO085 传感器数据测试 | ✅ 完成 | 旋转矢量、加速度计、陀螺仪数据流正常 |
| 4 | ADS1115 I2C 检测 | ✅ 完成 | 0x48 和 0x49 均检测成功，配置寄存器正常 |

### 设备端口映射

| 设备 | 端口 | 芯片 | MAC |
|------|------|------|-----|
| ESP32-P4 | /dev/ttyACM0 | ESP32-P4 v1.3 | 30:ed:a0:e2:24:b7 |
| ESP32-S3 | /dev/ttyACM1 | ESP32-S3 v0.2 (8MB PSRAM) | 30:30:f9:21:d8:dc |
...
3. ✅ **BNO085 传感器数据测试**：完成 — 旋转矢量、加速度计、陀螺仪数据流正常
4. ✅ **ADS1115 I2C 检测**：完成 — 0x48 和 0x49 均检测成功，配置寄存器正常
5. **ADS1115 原始 ADC 读取**：验证模拟输入读数（已知电压）
6. **弯曲传感器集成**：连接弯曲传感器 → 验证 ADC 读数 → 校准
7. **完整传感器集成**：BNO085 + ADS1115 + 弯曲传感器一起测试
8. **C6 刷写**：使用 USB-TTL 模块（3.3V!）刷写 C6 固件
9. **C6→P4 UART 链路**：验证端到端数据流
10. **模型导出**：运行 `python glove_firmware/scripts/export_model.py`（需要训练好的权重）
11. **LVGL BSP**：集成 P4 EV Board 7 寸 MIPI-DSI 显示屏
12. **ES8311 音频**：通过 `esp_codec_dev` BSP 组件连接 I2S
13. **竞赛演示**：全系统集成 + 演示脚本
...
### S3 I2C 验证详情 (2026-06-12, 更新于 2026-06-22)
...
### ADS1115 I2C 检测详情 (2026-06-22)
- **诊断固件**: `glove_firmware/test/test_ads1115/test_ads1115_diagnostic.ino`
- **PlatformIO 环境**: `[env:ads1115-diag]`
- **I2C 扫描结果**:
  - 0x48: ADS1115 #1 (ADDR→GND) — 配置寄存器 0x8583 (正常)
  - 0x49: ADS1115 #2 (ADDR→VDD) — 配置寄存器 0x8583 (正常)
  - 0x4B: BNO085 IMU — 检测成功
- **设备总数**: 3/3 — 全部检测成功
- **状态**: 通过 — I2C 总线完全正常，所有传感器就绪
- **下一步**: Phase 2 — 原始 ADC 读取测试（已知电压验证模拟输入）
